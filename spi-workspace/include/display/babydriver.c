...
void bd_set_pixel(int x, int y, bool state) {
  // bounds set
  if (x < 0 || x >= BD_FB_WIDTH || y < 0 || y >= BD_FB_HEIGHT) {
    return;
  }

  // NOTE: This was originally for a horizontal screen layout.
  // The SSD1309 uses a VERTICAL page addressing mode, of which there are 8.

  uint8_t page = y >> 3; // y / 8
  // uint8_t bit = y % 8;
  uint8_t bit = y & 7;
  uint16_t pos = page * 128 + x;

  if (state) {
    BD_FRAMEBUFFER[pos] |= (1 << bit);
  } else {
    BD_FRAMEBUFFER[pos] &= ~(1 << bit);
  }
...
int bd_get_pixel(int x, int y) {
  ...
  */
  ...
  return (BD_FRAMEBUFFER[pos] & (1 << bit)) ? 1 : 0;
}
